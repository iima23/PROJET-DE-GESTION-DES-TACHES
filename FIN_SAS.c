...
tache T[100];                         //on a declare un tableau de type tache                 
...
void supprimerTache() {
    ...
    if (pos >0 && pos <= nbretaches) {          //  condition pour pouvoir retirer une des taches
...
void triCroissant() {
    ...
    for (i = 0; i < nbretaches ; i++) {        
        for (j = i+1; j < nbretaches ; j++) {    
            if (T[i].annee_d_echeance > T[j ].annee_d_echeance){
                temp = T[i];                       //la var temp prend le contenu de T[i]et le stocker dans T[j]
                T[i] = T[j ];    
                T[j ] = temp;
            }
        }
